...
using JuMP, OptimalTransport, GLPK
...
ax = ones(Nx) / Nx; ay = ones(Ny) / Ny;

# ╔═╡ 43198426-f1bc-4e04-b34b-b3550f2d80d9
C = rand(Nx, Ny);
...
function emd_jump(ax, ay, C)
	...
	@constraints(
		model,
		begin
			[i in 1:Nx], sum(P[i,:]) == ax[i]
			[j in 1:Ny], sum(P[:,j]) == ay[j]
		end
	)
...
OptimalTransport.emd2(ax, ay, C, GLPK.Optimizer()) ≈ emd_jump(ax, ay, C)
