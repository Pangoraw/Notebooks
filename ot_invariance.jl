...
# ╔═╡ a33c9c52-0e0a-4225-abf2-34ee411f2e2a
using Distributions, PlutoUI

# ╔═╡ fc299b53-dea7-4585-b776-1dc861794c9e
using Plots, LinearAlgebra, Distances

# ╔═╡ c59733a6-a74d-4def-a29e-b986829925fb
using OptimalTransport

# ╔═╡ c6b158a0-aa8c-11ec-3357-0343f819b560
md"""
## Optimal Transport with Global Invariances
"""

# ╔═╡ 95393b8c-775a-4f93-8da0-32f19afa140d
import PlutoLinks

# ╔═╡ 1bff677e-a3f7-40bc-ba56-dcdf8bb01532
cost(X, Y) = - X' * Y

# ╔═╡ 022b2dae-4448-40ca-a16d-4ab6f048c90f


# ╔═╡ 7c3eb2b0-3bed-4226-a1b1-5c39c37b63a8
"""
	giot(X::AbstractArray, p::AbstractVector, Y::AbstractArray, q::AbstractVector)
...
function giot(
	X, p,
	Y, q;
	kₚ = 1, λ₀ = 5., λₘ = .1, η = .5, pᵢₙᵥ = 1_000, n_iters = 20
)
	d = size(X, 1)
	U, Σ, V = svd(rand(d, d))
	s = kₚ .* Σ ./ norm(Σ, pᵢₙᵥ)
	...
	for _ in 1:n_iters
		Cₚ = cost(X, P * Y)
		Γ = sinkhorn(p, q, Cₚ, λ, SinkhornStabilized())
		U, Σ, V = svd(X * Γ * Y')
		σ = Σ .^ (qq - 1)
		s = kₚ .* σ ./ norm(σ, pᵢₙᵥ)
		P = U * Diagonal(s) * V'
...
# ╔═╡ 26c75b48-47e0-4cad-be53-00aeff770ac1
begin
	Dx = MvNormal([1., 1.], [
		3. -2.
		-2. 3.
...
# ╔═╡ 83787c62-d682-41b8-af40-ac8b09b71c89
@bind N Slider(1:100, default = 20, show_value = true)

# ╔═╡ ad43f56f-4ff1-43fa-b2b7-f31e60c504eb
@bind n_iters Slider(20:100, show_value = true)

# ╔═╡ e4a962b4-8657-4f90-84de-8d4664103bbf


# ╔═╡ 1406b727-4e04-4182-9d27-1aa64b6427f6
function build_quiver(X, Y, γ)
	@assert size(X, 1) == size(Y, 1)

	Nx = size(X, 2)
	Ny = size(Y, 2)
	quiver = Array{eltype(X), 3}(undef, size(X, 1), Ny, Nx)
	
	for ((i, x), (j, y)) in
		Iterators.product(enumerate(eachcol(X)), enumerate(eachcol(Y)))

		quiver[:, j, i] = y - x
	end

	points = repeat(X, inner = (1, size(Y, 2)))
	quivers = reshape(quiver, :, Nx * Ny)
	linewidth = Nx .* reshape(γ, Nx * Ny)
	points, quivers, linewidth
end

# ╔═╡ 7da8dac5-b826-4a7b-be7a-89bdb8bf7212
md"""
### Co-Optimal-Transport
"""

# ╔═╡ 25a55993-0d69-457c-a4c3-a974bcc02d17
unif_like(X, size) = fill!(similar(X, size), 1/prod(size))

# ╔═╡ c339d5aa-6cec-48b7-bbc3-1920c7ef7eb9
unif_like(X, n::Int) = unif_like(X, (n,))

# ╔═╡ 89b8cd58-829c-4a59-b435-944736e0e1e6
unif(n::Int) = ones(n) ./ n

# ╔═╡ 647da1aa-8430-4a05-90a8-e8cdfac46c82
begin
	squeeze(x; dims) = reshape(x, Tuple(size(x, i) for i in 1:ndims(x) if i ∉ dims))
	squeeze(x) = squeeze(x; dims = findall(==(1), size(x)))
end

# ╔═╡ c9f03a87-a89b-4cec-a5df-891b88bad777
unsqueezed_dims(s, dims) = ntuple(i -> i ∈ dims ? 1 : s[i-count(<(i), dims)], length(dims) + length(s))

# ╔═╡ 26dc1b03-608c-4003-a50f-385d6b1b3704
"Adds extra dimensions with size 1"
unsqueeze(x::AbstractArray; dims) = reshape(x, unsqueezed_dims(size(x), dims))

# ╔═╡ 87fa3d20-1fbe-4ea9-b86c-2bb71cda1b03
ones_like(X, dims) = fill!(similar(X, dims), one(eltype(X)))

# ╔═╡ b1a9f4fa-fe43-41d7-a74e-757114f1a672
ones_like(X, dims...) = ones_like(X, (dims...,))

# ╔═╡ 6f03132f-f723-46b4-ad56-f6fd8863e051
ones_like(X) = ones_like(X, size(X))

# ╔═╡ ed542311-d1f7-431d-810b-942b6b6fc4e8
...
# ╔═╡ 802b44a7-fe79-4491-b35e-54d46a7c8d4a
...
# ╔═╡ d667dadd-e2d4-4272-ac8b-78d8129f5c98
begin
	"""
	...
	> [1]G. Peyré, M. Cuturi, et J. Solomon, « Gromov-Wasserstein Averaging of Kernel and Distance Matrices », in Proceedings of The 33rd International Conference on Machine Learning, june 2016, p. 2664‑2672.
	"""
...
function coot(
    ...
    metric = L²,
	ϵ = 0.1,
    otplan = (μ, ν, C) -> sinkhorn(μ, ν, C, ϵ, SinkhornStabilized()),
)
    πˢ = w * w′'
...
# ╔═╡ 874ffab1-a166-4c49-97fb-549dc84f2e1c
# ╠═╡ show_logs = false
begin
	λ = .3
	D = 5
	...
	Γ, P, ps = giot(X, q, Y, p; n_iters, pᵢₙᵥ =1000)

	cost_giot = sum(cost(X, P * Y) .* Γ)

	C = cost(X, Y)
	γ = sinkhorn(q, p, C, λ, SinkhornStabilized())
	cost_ot = sum(γ .* C)

	πᵛ, πˢ, ps_coot = coot(X, Y; tol = 0., ϵ = λ, n_iters)
	cost_coot = sum(cost(πᵛ * X, Y) .* πˢ)
	
	cost_giot, cost_coot, cost_ot
end

# ╔═╡ a2956a99-3bd5-4d03-8a77-96a5eb6196bb
let
	scatter(eachrow(X)..., label = "X", opacity = .4)

	scatter!(eachrow(X * (Γ .* size(X, 2)))..., label = "XΓ")
	scatter!(eachrow(X * (γ .* size(X, 2)))..., label = "Xγ")

	scatter!(eachrow(Y)..., label = "Y", opacity = .4)
	scatter!(eachrow(P * Y)..., label = "PY")
end

# ╔═╡ 60eb45aa-26ce-404b-a4c0-763fbcca7d46
# ╠═╡ show_logs = false
@PlutoLinks.use_memo([cost_giot]) do
	xlims = extrema(vcat(X[1,:], Y[1,:])) .+ (-.1, .1)
	ylims = extrema(vcat(X[2,:], Y[2,:])) .+ (-.1, .1)
	@gif for (i, P) in enumerate(ps) #[3:100]
		plot(; xlims, ylims, title = "Invariant OT | Iteration $i")
		
		Ŷ = P * Y
		scatter!(eachrow(X)..., label = "X")
		scatter!(eachrow(Ŷ)..., label = "PᵢY")
		scatter!(eachrow(Y)..., label = "Y", opacity = .4)
		
		p, uv, lw = build_quiver(X, Ŷ, Γ)
		quiver!(eachrow(p)..., quiver = Tuple(eachrow(uv)), opacity = lw)
	end
end

# ╔═╡ 580c6bbf-9daf-4bd8-941d-9c79ccfe9640
# ╠═╡ show_logs = false
let
	xlims = extrema(vcat(X[1,:], Y[1,:])) .+ (-.1, .1)
	ylims = extrema(vcat(X[2,:], Y[2,:])) .+ (-.1, .1)
	@gif for (i, P) in enumerate(ps_coot) #[3:100]
		plot(; xlims, ylims, title = "Co-Optimal Transport | Iteration $i")
		
		Ŷ = P * Y
		scatter!(eachrow(X)..., label = "X")
		scatter!(eachrow(Ŷ)..., label = "πᵛᵢY")
		scatter!(eachrow(Y)..., label = "Y", opacity = .4)
...
# ╔═╡ 82b12ec2-07ff-42f9-9ee6-6837130edd9f
# ╠═╡ show_logs = false
let
	scatter(eachrow(X)...)
	scatter!(eachrow(Y)...)

	p, uv, lw = build_quiver(X, Y, γ)
	quiver!(eachrow(p)...; quiver = eachrow(uv) |> Tuple, opacity = lw)
end

# ╔═╡ 56e6bae9-7977-4b07-833b-85381d8d9fb5
# ╠═╡ show_logs = false
let
	scatter(eachrow(X)..., label = "X")
	scatter!(eachrow(P * Y)..., label = "PY")

	p, uv, lw = build_quiver(X, P * Y, Γ)
	quiver!(eachrow(p)..., quiver = Tuple(eachrow(uv)), opacity = lw)
end

# ╔═╡ cba4c9d0-f368-45c6-9134-3fd1fe1eea01
let
	scatter(eachrow(Y)..., label = "Y")
	scatter!(eachrow(P * Y)..., label = "Y transformed")
	scatter!(eachrow(X)..., label = "X")
end
...
manifest_format = "2.0"
project_hash = "8a6eeb8014ee7c6c8df5ee70b41dec888305bb32"
