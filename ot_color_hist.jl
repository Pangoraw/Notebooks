...
"""
	emd(a::Vector{Float::64}, b::Vector{Float64}, C::Matrix{Float64})

Exact Earth Mover Distance (EMD).
"""
function emd(a, b, C)
	...
	# Minimize the tranport cost
	@objective(model, Min, sum(
		P .* C
	))

	# Add the marginal constraints
	@constraints(
		model,
		begin
			[i in 1:Nμ], sum(P[i,:]) == a[i]
			[j in 1:Nν], sum(P[:,j]) == b[j]
		end
	)
	...
	if termination_status(model) != OPTIMAL
		error("Failed to find a solution (status = $(termination_status(model)))")
	end
...
# ╠═╡ show_logs = false
function transfer(img_a, img_b, n_clusters)
	μ = kmeans(img_a, n_clusters)
	ν = kmeans(img_b, n_clusters)

	γ = transport_hist(μ.centers, ν.centers) .* n_clusters

	new_centers = γ * ν.centers'
	colorview(RGB,
