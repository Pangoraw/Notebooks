...
# ╔═╡ 1e6f7319-e284-4be8-a7ba-fc0b9ee74f76
using PlutoUI
...
# ╔═╡ ec4aea3f-22e9-44c2-9bec-756e6f70407f
@bind θ Slider(0:0.1:2π, default=π/4)

# ╔═╡ eec6a88d-aed2-4c30-b421-402f90e98983
@bind a Slider(-2.:0.1:2., default=0., show_value=true)

# ╔═╡ 91a4f644-a936-471d-b73a-b8b2930f4230
radius= (1 + tanh(a / 2)) / 2
...
\\begin{tikzpicture}
	...
	$(
		join([let radius =  (1 + tanh(a / 2)) / 2,
				center = (
					(1 - radius) * cos(θ),
					(1 - radius) * sin(θ)
				)
			
			"\\draw[gray] ($(scale * center[1]), $(scale * center[2])) circle ($(scale * radius)) node[left=$(max(-1,a))] {\$a = $(round(a,digits=2))\$};"
		end for a in (-1, 0., 1.0)], "\n\n")
	)

	\\node[circle,draw,fill=blueish] (ideal) at ($(scale * cos(θ)), $(scale * sin(θ))) {\$\\mathrm p\$};
