...
# ╔═╡ 8d8cd846-b1c2-11ec-123b-5df5e8075b11
using Zygote,
	Plots,
	LinearAlgebra,
	OptimalTransport,
...
# ╔═╡ 8ad67277-b9ac-4e6d-a682-83def1edc95c
md"""
Rotation invariant Optimal Transport

> (1) Alvarez-Melis, D.; Jegelka, S.; Jaakkola, T. S. Towards Optimal Transport with Global Invariances. arXiv:1806.09277 [cs, stat] 2019.
"""

# ╔═╡ 4820ac0d-fbeb-4724-9e16-28604292f519
@bind θ Slider(0:.01:2π, default=π/2, show_value=true)

# ╔═╡ 19eb9096-356a-4b32-8392-1ce714f450e9
unwrap(x) = first.(x), getindex.(x, 2), last.(x)

# ╔═╡ d14ba90a-42e9-4b4f-8648-b18a95b7df81
N, D = 10, 3;

# ╔═╡ 38065b95-b029-45ca-ad0a-9e9aa94fcef1
@bind λs RangeSlider(.01:.01:10.)

# ╔═╡ de87539e-a01e-44c7-9209-73592e4316ef
X = 1.2 * rand(D, N);

# ╔═╡ 1df09479-a4c3-427c-baf5-bf4e2a1a2282
λₑ, λₛ = extrema(λs)

# ╔═╡ fbbd1c25-bc68-491b-93ea-e015bc2e22da
function rotation_invariant_ot(a, X, b, Y)
	d = size(X, 1)
	...
	for _ in 1:n_iters
		C = - X' * Y
		Γ = sinkhorn(a, b, C, λ, SinkhornStabilized(); maxiter = 40)
		U, _, V = svd(X * Γ * Y')
...
# ╔═╡ d1e94a62-c11a-4640-bdc1-fdc01e82305b
R = [
	cos(θ) sin(θ) 0
	-sin(θ) cos(θ) 0
	0 0 1
...
# ╔═╡ 19aa4025-44cf-457c-8335-8b9531333435
a = b = fill(1/N, (N,));

# ╔═╡ 12b38466-3661-48dc-b86f-e22cab4a3490
begin
	perm = randperm(N)
	Y = (R * X[:,perm])
	C = pairwise(sqeuclidean, X, Y)
	P, Γ = rotation_invariant_ot(a, X, b, Y)
	Cc = pairwise(sqeuclidean, X, P * Y)
	emd2(a, b, C, Tulip.Optimizer()),
		sum(Γ .* Cc)
end

# ╔═╡ 6a457bc2-d8f6-4cfe-9ecb-1e7dd8707239
D ∉ (2, 3) ? nothing : let
	PY = P*Y
	scatter(eachrow(X)..., label = "X")
	scatter!(eachrow(Y)..., label = "RX")
	scatter!(eachrow(PY)..., label = "PRX")
	...
	quiver!(unwrap(x)..., quiver = unwrap(v), alpha = N * γ)
	
	title!("Rotation invariant Optimal Transport")
end

# ╔═╡ 03acdfd7-f4ba-48db-a705-501617a99570
get_costs(θ) = begin
	R = [
		cos(θ) sin(θ) 0
		-sin(θ) cos(θ) 0
	...
	Cc = pairwise(sqeuclidean, X, P * Y)
	emd2(a, b, C, Tulip.Optimizer()),
		sinkhorn2(a, b, C, .1, SinkhornStabilized()),
		sum(Γ .* Cc)
end

# ╔═╡ f8e9a013-9a32-4efe-aac9-9dadbf92a425
let
	θs = 0:.1:2π
	costs = θs .|> get_costs
	emd, sink, riot = first.(costs), getindex.(costs, 2), last.(costs)
	
	plot(emd, label = "OT")
	plot!(sink, label = "Sinkhorn")
	plot!(riot, label = "RIOT")
	title!("Optimal Transport with rotations")
	xaxis!("θ")
	yaxis!("Transport cost")
end
