...
# ╔═╡ 4d3b392a-2db6-11ed-00a4-4d78fb41f29b
using Plots, Distances, LinearAlgebra, PlutoUI, MarkdownLiteral,
	  PlutoHooks
...
```math
\inf_\gamma \langle\gamma,C\rangle + \epsilon\Omega(\gamma) + \lambda_1D(\gamma1\mid\mu) + \lambda_2D(\gamma^\top1\mid\nu)
```

###### Proximal Operator
```math
\text{prox}^D_F(x) = \arg\min_{y\in E} F(y) + D(y\mid x)
```
###### Entropy Function
```math
\Omega: \mathbb R\rightarrow\mathbb R\cup\{\infty\}
```
###### Divergences
```math
\mathcal D_\phi(\mu\mid\nu) = \int_T\phi\left(\frac{d\mu}{d\nu}\right)d\nu + \phi^\prime_\infty\mu^\bot(T)
```
"""
...
function draw_quiver!(X, Y, γ)
	@assert size(γ) == (size(X, 2), size(Y, 2))

	x, y, u, v, α = [], [], [], [], []
	for i in axes(X, 2),
		j in axes(Y, 2)

		alpha = 3 * γ[i,j] / sum(@view γ[:,j])
		alpha < 1e-6 && continue
