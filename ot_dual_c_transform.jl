...
# ╔═╡ 84ad50e6-f233-11ec-052c-097e5edd711a
using JuMP, GLPK

# ╔═╡ 2af8710a-30a7-4abe-b1ec-f55628e06e54
function emd_dual_c_transform(a, b, C)
	Nx = length(a); Ny = length(b)

	# Create the JuMP model
	model = Model(GLPK.Optimizer)
	
	@variable(model, f[1:Nx] >= 0)

	# Create the objective which is the cost
	@objective(model, Max, sum(
		a[i] * f[i] for i in 1:Nx
	) + sum(
		b[j] * minimum(C[i,j] - f[i] for i in 1:Nx) for j in 1:Ny
	))


end

# ╔═╡ 9ecb68db-05eb-459e-b0e5-99ff7e0c13ec
let
	a = [.9, .1]
	b = a
	C = [.2 .1; 2. 3.]
	emd_dual_c_transform(a, b, C)
end
